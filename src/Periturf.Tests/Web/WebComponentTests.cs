...
namespace Periturf.Tests.Web
{
    ...
    class WebComponentTests
    {
        ...
        private Func<IWebRequest, bool> _predicate;
        ...
        public async Task SetupAsync()
        {
            ...
            _predicate = A.Fake<Func<IWebRequest, bool>>();
        ...
        public async Task Given_ConfiguredRequest_When_Process_Then_HandlersCalledAfterPredicate()
        {
            ...
            A.CallTo(() => _predicate.Invoke(A<IWebRequest>._)).Returns(true);
            ...
            A.CallTo(() => _predicate.Invoke(A<IWebRequest>._)).MustHaveHappened().Then(
        ...
        {
            var context = new DefaultHttpContext();
            A.CallTo(() => _predicate.Invoke(A<IWebRequest>._)).Returns(false);

            await _sut.ProcessAsync(context);

            A.CallTo(() => _eventHandlerFactory.Create(A<IEnumerable<IEventHandlerSpecification<IWebRequest>>>.That.Contains(_handlerSpec))).MustHaveHappened();

            A.CallTo(() => _predicate.Invoke(A<IWebRequest>._)).MustHaveHappened();
