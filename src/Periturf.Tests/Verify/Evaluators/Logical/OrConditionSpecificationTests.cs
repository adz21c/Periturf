...
{
    [TestFixture]
    class OrConditionSpecificationTests
    {
        [Test]
        public async Task Given_ChildConditions_When_BuildEvaluator_Then_CreateChildEvaluatorsAndReturnParent()
        {
            ...
            var spec = new OrConditionSpecification(new List<IConditionSpecification> { condition, condition2 });
            ...
            Assert.AreEqual(typeof(OrConditionEvaluator), parentEvaluator.GetType());
